...
public abstract class AbstractSmartDocumentModel extends AbstractDocumentMapModel {
	...
	/**
	 * Constructor, calling overloaded constructor without ID and in editMode
	 *
	 * Remember to call setTypeConversionMap() with a Map of any Items that need
	 * type conversion (e.g. Dates, HashMaps etc)
	 */
	 ...
	 */
	public AbstractSmartDocumentModel(String documentId, boolean editMode) {
		super(documentId);
	...
	public Document getBEDoc() {
		...
		if (StringUtils.isNotEmpty(getUnid())) {
			retVal_ = GenericDatabaseUtils.getDocumentByNoteID_Or_UNID(getUnid());
		}
		return retVal_;
	}
	...
	/**
	 * Gets a field value cast to a String, returning a blank String if the
	 * field does not exist or an error occurs
	 *
	 * @param name
	 *            String Item name to get value from
	...
	/**
	 * Gets a field value cast to a Date, returning null if the field is a
	...
	/**
	 * Gets a Date/Time field value as a Date, returning in date only or date
	 * and time format using {@link AppUtils.convertDateAndTime()} or
	 * {@link AppUtils.convertDate()}
	 *
	 * @param name
	 *            String Item name to get value from
	 * @param includeTime
	...
	public String getDateString(final String name, final boolean includeTime) {
		...
		try {
			...
			if (Date.class.equals(getValue(name).getClass())) {
				...
				final DateTime dtTime = getTypedValue(name, DateTime.class);
	 ...
	 * (non-Javadoc)
	 *
	 * @see com.timtripcony.AbstractMapModel#setValue(java.lang.Object,
	 * java.lang.Object)
	 */
	@Override
	 ...
	 * Getter for editable
	 *
	 * @return boolean whether or not the document is in edit mode (because
	 *         muscle memory makes me use "editable"
	 */
	...
	public boolean isBlank(String itemName) {
		...
		try {
			final Object valueToCheck = this.getValue(itemName);
			if (null == valueToCheck) {
				...
				// This could be refactored to check
				// "[]".equals(vector.toString() - But I've changed setValue()
				// to create a String instead of a Vector, so not sure how to
				// test this
	 ...
	 * @param forceDecimals
	 *            boolean whether or the currency value should force decimals
	 *            even if a whole number
	...
	public String convertToCurrency(Object value, boolean forceDecimals) {
		...
		try {
			if (value instanceof Number) {
				final float number = new Float(value.toString());
				final float epsilon = 0.004f; // 4 tenths of a penny
	 ...
	 * @return boolean whether or not the Item exists in this wrapper of the
	 *         Document
	 */
